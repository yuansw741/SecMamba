...
class CustomGraphDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        for index, row in df.iterrows():
            ...
            G.add_edge(row['IPV4_SRC_ADDR'], row['IPV4_DST_ADDR'], edge_attr=edge_features, label=row['label'])
...
# 使用示例
# test_loss, test_acc, test_precision, test_recall, test_f1 = test(test_loader)
...
num_epochs = 20
with open(f'../../code/pythonProject1/超参数选择/NFBot/{num}', 'w', encoding='utf-8') as f:
...
# 最终测试
avg_loss, accuracy, precision, recall, f1 = test2(test_loader)
print(f"Test Loss: {avg_loss:.4f}, Test Accuracy: {accuracy:.4f}, Test Precision: {precision:.4f}, Test Recall: {recall:.4f}, F1: {f1:.4f}")